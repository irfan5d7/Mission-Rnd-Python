...
def generator_zip(seq1, seq2, *more_seqs):
    ...
    """
    iterables = []
    if hasattr(seq1, "__iter__"):
        iterables.append(iter(seq1))
    if hasattr(seq2, "__iter__"):
        iterables.append(iter(seq2))
    for seq in more_seqs:
        if hasattr(seq, "__iter__"):
            iterables.append(iter(seq))
    l = len(iterables)
    while True:
        item = tuple(next(x) for x in iterables)
        if len(item) == l:
            yield item
        else:
            raise StopIteration
